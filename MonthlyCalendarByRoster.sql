...
DELIMITER $$
DROP PROCEDURE IF EXISTS MonthlyCalendarByRoster$$
CREATE PROCEDURE MonthlyCalendarByRoster(IN startDate DATE)
BEGIN
	...
	WHILE (thisDay <= numDaysInMonth) DO
		INSERT INTO MonthTable
		   ...
		   (SELECT thisDay AS Today, Tasks.TaskName AS TaskName, Tasks.TaskID AS TaskID, Tasks.ProgramID AS ProgramID, 
			...
			HAVING ((PersonCount + SubCount - UnavailCount) < NumPersonsReqd 
				OR (LeaderCount - UnavailLdrCount) < NumLeadersReqd)
		   );
