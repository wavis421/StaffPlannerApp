...
DELIMITER $$
DROP PROCEDURE IF EXISTS MonthlyCalendarByTime$$
CREATE PROCEDURE MonthlyCalendarByTime(IN startDate DATE, IN timeFilter VARCHAR(150))
BEGIN
		...
		INSERT INTO MonthTable
		   ...
		   (SELECT thisDay AS Today, Tasks.TaskName AS TaskName, Tasks.TaskID AS TaskID, Tasks.ProgramID AS ProgramID, 
			...
			WHERE ((Tasks.DaysOfWeek & (1 << currDow)) != 0)
				...
				AND FIND_IN_SET(MAKETIME(Hour,Minute,0), timeFilter)
